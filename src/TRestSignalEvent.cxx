...
ClassImp(TRestSignalEvent)
    ...
    RESTWarning << "Creating legacy event TRestSignalEvent" << RESTendl;
    RESTWarning << "This event is now named TRestDetectorSignalEvent" << RESTendl;
    Initialize();
}
...
TPad* TRestSignalEvent::DrawEvent(const TString& option) {
    if (fPad != NULL) {
        delete fPad;
        fPad = NULL;
    }
